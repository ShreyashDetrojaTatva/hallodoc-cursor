...
import { AUTH_ENDPOINTS } from '@main/constants';
...
export class AuthService {
  ...
  login(usernameOrEmail: string, password: string): Observable<LoginResponse> {
    return this.http.post<LoginResponse>(AUTH_ENDPOINTS.LOGIN, { usernameOrEmail, password })
      .pipe(
        tap(response => {
  ...
  getProfile(): Observable<ProfileData> {
    return this.http.get<ProfileData>(AUTH_ENDPOINTS.GET_PROFILE);
  }

  updateProfile(data: Partial<ProfileData>): Observable<ProfileData> {
    return this.http.put<ProfileData>(AUTH_ENDPOINTS.UPDATE_PROFILE, data)
      .pipe(
        tap(() => {
  ...
  forgotPassword(email: string): Observable<ForgotPasswordResponse> {
    return this.http.post<ForgotPasswordResponse>(AUTH_ENDPOINTS.FORGOT_PASSWORD, { email }).pipe(
      tap(response => {
  ...
  resetPassword(resetData: { token: string; password: string }): Observable<ResetPasswordResponse> {
    return this.http.post<ResetPasswordResponse>(AUTH_ENDPOINTS.RESET_PASSWORD, resetData);
  }
} 